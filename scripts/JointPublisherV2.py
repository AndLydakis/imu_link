...
import rospy
from ImuPoint import ImuPointV2
...
def read_joints_from_params():
    ...
    while True:
        if rospy.has_param("/joint" + str(count) + "/name"):
            ...
            print(params)
            joints[params["name"]] = ImuPointV2(name_=params["name"],
                                                child_frame_=params["child_frame"],
                                                parent_frame_=params["parent_frame"],
                                                sub_topic_=params["sub_topic"],
                                                ipitch_=params["offset_pitch"],
                                                iroll_=params["offset_roll"],
                                                iyaw_=params["offset_yaw"],
                                                ix_=params["offset_x"],
                                                iy_=params["offset_y"],
                                                iz_=params["offset_z"]
                                                )
